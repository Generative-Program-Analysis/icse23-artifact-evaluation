...
- `headers` contains the source of GenSym's runtime, which are currently implemented as C++ headers. They will be linked together with the compiled application code.
...
- `src/main` is the root directory of GenSym's front-end comiler source code
  ...
  - `src/main/scala/gensym` is the main directory of GenSym's implementation
    - GenSym implements a few variants staged symbolic interpreters (i.e. compilers), which are contained in `src/main/scala/engines`. The default and most mature compile is `src/main/scala/engines/ImpCPSEngine.scala` that generates CPS code and uses in-place update when possible. If you want to understand how the staged symbolic
    interpreter works, you can start from this file.
- `src/test` contains the testing infrastructure that are used in Github CI
...
This experiment answers RQ4 and generates Table IV of the paper, which
compares the performance of KLEE's POSIX file system and our meta file system.
These two file systems are then used together with application code compiled by GenSym.
...
- `t-solver-ext` and `t-solver-int` are external and internal solver time used
