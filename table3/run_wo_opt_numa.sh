...
gs_option="--output-tests-cov-new --search=random-path --solver=z3 --output-ktest --timeout=7200 --max-sym-array-size=4096 --print-detailed-log=2 --no-cex-cache --no-br-cache --no-cons-indep"
...
running_gs() {
  ...
  for id in "${!orders[@]}"; do
    ...
    command_gs="numactl -N0 -m0 ./${gs_bin} ${gs_option} --symloc-strategy=${sym_strategy} --thread=$th"
    ...
    bash command.sh
    echo $program,$th,`python3 ../../extract_time.py ${curr_name}_raw.log` >> ../../result_no_opt.csv
    cd ..
  done
